...
class A_Langchain_Model(BaseQueryGenerator):
    """
    ...
    def __init__(self, stopword_file,prompt_file,n=3,provider='ollama',model='mistral',temperature=1.0,verbose=False, background_file=[]):
        super(BasicLangChainQueryGenerator, self).__init__(stopword_file, background_file=background_file, allow_similar=False)
        prompt_template = ""
        ...
        self._result_schema = [ResponseSchema(
            name="queries",
            description=f"You will be given a topic, with a title and description. The title should be more important than the description. Generate an array of 30 possible queries for the topic of less than or equal to {n} search terms ordered by likelihood of success.",
            type="list"
            )]
